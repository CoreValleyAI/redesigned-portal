...
const INTERESTS = [
  ...
  { value: "model-endpoints", label: "Model endpoints (inference)" },
...
export function ContactForm() {
  ...
  if (status === "sent") {
    return (
      <Card padding={32}>
        <Icon name="check-circle" size={28} weight="duotone" className="text-ink-100" />
    ...
    <Card padding={32}>
      ...
      <form ref={formRef} onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        <p className="text-center text-xs text-ink-500">
          Or email{" "}
          <a href={`mailto:${INBOX}`} className="text-hydro underline decoration-hydro/40 underline-offset-4 transition-colors duration-normal hover:text-hydro-300">
            {INBOX}
          </a>{" "}
          directly.
        </p>
